/**
 * Simple Flight Logistics Automator
 * Creates calendar events for a flight anchor event with #flightanchor in the title.
 * Events: Board at Gate, Walk to Gate, United Club near Gate, Walk to United Club near Gate, Security, Deplane.
 */

function FlightLogisticsAutomator() {
  var calendar = CalendarApp.getDefaultCalendar();
  var now = new Date();
  var searchPeriod = new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000); // 2 weeks ahead
  ...
  flightAnchors.forEach(function(flight) {
    ...
    var location = flight.getLocation() || '';

    // Parse airport codes from title
    var routeMatch = /\b([A-Z]{3})\s*(?:TO|->|-)?\s*([A-Z]{3})\b/i.exec(originalTitle);
    var origin = routeMatch ? routeMatch[1] : 'ORIGIN';
    var arrival = routeMatch ? routeMatch[2] : 'ARRIVAL';

    var anchorTag = '#flightanchor';
    ...
    timeline.forEach(function(item) {
      var eventStart = item.mins > 0 ? new Date(startTime.getTime() - item.mins * 60000) : new Date(endTime.getTime() - item.mins * 60000);
      var eventEnd = new Date(eventStart.getTime() + item.dur * 60000);
      calendar.createEvent(item.name, eventStart, eventEnd, {location: location});
    });
  });
}
